...
class IsTeacherV2(BasePermission):
    def has_permission(self, request, view):
        if bool(request.user and request.user.is_authenticated) and request.user.is_teacher():
            return True
        return False


class IsExecutiveV2(BasePermission):
    def has_permission(self, request, view):
        if bool(request.user and request.user.is_authenticated) and request.user.is_executive():
            return True
        return False


class IsAdminV2(BasePermission):
    def has_permission(self, request, view):
        if bool(request.user and request.user.is_authenticated) and request.user.is_admin():
            return True
        return False


class IsExecutiveAdminV2(BasePermission):
    def has_permission(self, request, view):
        staff = request.user
        if bool(staff and staff.is_authenticated) and (staff.is_executive() or staff.is_admin()):
            return True
        return False
...
class IsStaffV2(BasePermission):
    """
    is_authenticated da kontrol ediyor..
    """
    def has_permission(self, request, view):
        staff = request.user
        if bool(staff and staff.is_authenticated) and (staff.is_teacher() or staff.is_executive() or staff.is_admin()):
            return True
        return False


class CanEditUserV2(IsStaffV2):
    def has_object_permission(self, request, view, obj):
        staff = request.user
        if staff == obj:
            return True
